...
class DataPreparation:
    """
    Class that prepares data and return X(Y)_train,test as pandas DataFrames
    """

    def __init__(self, path=".", GA_selection=False):
    ...
    def fetch_data(self, sample):
        if sample == "titanic":
            ...
            data_set = pd.read_csv(
                self.workpath + "/datasets/breast_cancer.csv")
        elif sample == "german":
            ...
            file_train = uproot.open(
                self.workpath + "/datasets/train_D02k3pi.root")
            data_train = file_train["d0tree"].arrays(library="pd")
            file_test = uproot.open(
                self.workpath + "/datasets/test_D02k3pi.root")
            data_test = file_test["d0tree"].arrays(library="pd")
            return (data_train, data_test)
        elif sample == "belle2_iv":
            file_train = uproot.open(
                self.workpath +
                "/datasets/train_D02kpipi0vxVc-cont0p5.root")
            #file_train = uproot.open(self.workpath+"/datasets/test_belle2_iv.root")
            data_train = file_train["d0tree"].arrays(library="pd")
            file_test = uproot.open(
                self.workpath +
                "/datasets/test_D02kpipi0vxVc-cont0p5.root")
            data_test = file_test["d0tree"].arrays(library="pd")
            # return data_train
            return (data_train, data_test)
    ...
    def dataset(
        ...
        if not sampling:
            ...
            train_test = isinstance(data_temp, tuple)
            if not train_test:
                data_set = data_temp
        ...
        if sample_name == "titanic":
            ...
            train_test = True
            X_train, Y_train, X_test, Y_test = self.belle2_3pi(
                data_train, data_test, sampling, sample_name=sample_name)
        elif sample_name == "belle2_iv":
            train_test = True
            X_train, Y_train, X_test, Y_test = self.belle2_iv(
                data_train, data_test, sampling, sample_name=sample_name)
        elif sample_name == "belle2_challenge":
            train_test = True
            X_train, Y_train, X_test, Y_test = self.belle2_challenge(
                data_train, data_test, sampling, sample_name=sample_name)
        ...
        if indexes is None:
            if not train_test:
                X_train, X_test, Y_train, Y_test = train_test_split(
                    X, Y, test_size=0.1, random_state=2)  # split_sample
        else:
            if not train_test:
                X_train, X_test, Y_train, Y_test = self.indexes_split(
                    X, Y, split_indexes=indexes, train_test=train_test)
            else:
                X_train, X_test, Y_train, Y_test = self.indexes_split(
                    X_train, Y_train, X_test, Y_test, split_indexes=indexes, train_test=train_test)
    ...
    def belle2(self, data_set, sampling, sample_name):
        ...
        if (sampling or self.genetic):  # Sampling was already carried, don"t sample again!
            ...
            data_set = pd.DataFrame(
                MinMaxScaler().fit_transform(data_set), columns=cols)
    ...
    def belle2_iv(self, data_train, data_test, sampling, sample_name):
        ...
        if (sampling or self.genetic):  # Sampling already done or not needed
            ...
            data_train = pd.DataFrame(
                MinMaxScaler().fit_transform(data_train), columns=cols)
            ...
            data_test = pd.DataFrame(
                MinMaxScaler().fit_transform(data_test), columns=cols)
        ...
        sampled_data_train = resample(
            data_train,
            replace=False,
            n_samples=1000,
            random_state=None)
        sampled_data_test = resample(
            data_test,
            replace=False,
            n_samples=1000,
            random_state=None)
        ...
        # Data scaling [0,1]
        sampled_data_train = pd.DataFrame(
            MinMaxScaler().fit_transform(sampled_data_train), columns=cols)
        sampled_data_test = pd.DataFrame(
            MinMaxScaler().fit_transform(sampled_data_test), columns=cols)
    ...
    def titanic(self, data_set):
        ...
        # Set titles, these are quite problematic, so we format using the old
        # way
        data_set.loc[:, "Title"] = data_set.Name.str.extract(
            "([A-Za-z]+)", expand=False)
        ...
        data_set.loc[(data_set["Age"] > 16) & (
            data_set["Age"] <= 32), "Age"] = 1
        data_set.loc[(data_set["Age"] > 32) & (
            data_set["Age"] <= 48), "Age"] = 2
        data_set.loc[(data_set["Age"] > 48) & (
            data_set["Age"] <= 64), "Age"] = 3
        data_set.loc[data_set["Age"] > 64, "Age"] = 4

        # Combine and drop features
        data_set["FamilySize"] = data_set["Siblings/Spouses Aboard"] + \
            data_set["Parents/Children Aboard"] + 1
    ...
    def bCancer(self, data_set):
        ...
        dummy = [
            ...
            "55-59"]
        for i in range(len(dummy)):
            data_set.loc[data_set["tumorSize"].astype(
                str) == dummy[i], "tumorSize"] = i
        ...
        dummy = [
            ...
            "36-39"]
        for i in range(len(dummy)):
            data_set.loc[data_set["invNodes"].astype(
                str) == dummy[i], "invNodes"] = i
    ...
    def german(self, data_set):
        ...
        for i in range(len(dummy)):
            data_set.loc[data_set["SexStatus"].astype(
                str) == dummy[i], "SexStatus"] = i
        ...
        for i in range(len(dummy)):
            data_set.loc[data_set["Plans"].astype(
                str) == dummy[i], "Plans"] = i
    ...
    def solar(self, data_set):
        ...
        dummy = ["A", "B", "C", "D", "E", "F", "H"]
        for i in range(len(dummy)):
            data_set.loc[data_set["Zurich"].astype(
                str) == dummy[i], "Zurich"] = i
        ...
        dummy = ["X", "O", "I", "C"]
        for i in range(len(dummy)):
            data_set.loc[data_set["Distro"].astype(
                str) == dummy[i], "Distro"] = i

        # Create the Class
        data_set["Class"] = data_set.sum(axis=1)
        data_set.loc[(data_set["Class1"] == 0) & (data_set["Class2"] == 0) & (
            data_set["Class3"] == 0), "Class"] = +1
        data_set.loc[(data_set["Class1"] != 0) | (data_set["Class2"] != 0) | (
            data_set["Class3"] != 0), "Class"] = -1
    ...
    def car(self, data_set):
        ...
        dummy = ["vhigh", "high", "med", "low"]
        for i in range(len(dummy)):
            data_set.loc[data_set["Maint"].astype(
                str) == dummy[i], "Maint"] = i

        dummy = ["2", "3", "4", "5more"]
        for i in range(len(dummy)):
            data_set.loc[data_set["Doors"].astype(
                str) == dummy[i], "Doors"] = i

        dummy = ["2", "4", "more"]
        for i in range(len(dummy)):
            data_set.loc[data_set["Persons"].astype(
                str) == dummy[i], "Persons"] = i
        ...
        dummy = ["low", "med", "high"]
        for i in range(len(dummy)):
            data_set.loc[data_set["Safety"].astype(
                str) == dummy[i], "Safety"] = i

        dummy = ["acc", "good", "vgood"]
        for i in range(len(dummy)):
            data_set.loc[data_set["Class"].astype(
                str) == dummy[i], "Class"] = 1
    ...
    def adult(self, data_set):
        ...
        dummy = [
            ...
            "Never-worked"]
        for i in range(len(dummy)):
            data_set.loc[data_set["workclass"].astype(
                str) == dummy[i], "workclass"] = i
        ...
        dummy = [
            ...
            "Preschool"]
        for i in range(len(dummy)):
            data_set.loc[data_set["education"].astype(
                str) == dummy[i], "education"] = i
        ...
        dummy = [
            ...
            "Married-AF-spouse"]
        for i in range(len(dummy)):
            data_set.loc[data_set["marital-status"].astype(
                str) == dummy[i], "marital-status"] = i
        ...
        dummy = [
            ...
            "Armed-Forces"]
        for i in range(len(dummy)):
            data_set.loc[data_set["occupation"].astype(
                str) == dummy[i], "occupation"] = i
        ...
        dummy = [
            ...
            "Unmarried"]
        for i in range(len(dummy)):
            data_set.loc[data_set["relationship"].astype(
                str) == dummy[i], "relationship"] = i
        ...
        dummy = [
            ...
            "Holand-Netherlands"]
        for i in range(len(dummy)):
            data_set.loc[data_set["native-country"].astype(
                str) == dummy[i], "native-country"] = i
