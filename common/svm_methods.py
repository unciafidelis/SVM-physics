...
class LinearPrecomputed():
    ...
    def explicit_calc(self):
        return np.array([[self.linear_explicit(i, j)
                        for j in self.y] for i in self.x])
    ...
    def numpy_calc(self):
        return np.array([[self.linear_np(i, j) for j in self.y]
                        for i in self.x])
...
class RBFPrecomputed():
    ...
    def explicit_calc(self):
        return np.array([[self.rbf_explicit(i, j)
                        for j in self.y] for i in self.x])
...
class PolyPrecomputed():
    ...
    def explicit_calc(self):
        return np.array([[self.poly_explicit(i, j)
                        for j in self.y] for i in self.x])
