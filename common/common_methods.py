...
def roc_curve_adaboost(Y_thresholds, Y_test) -> tuple:
    ...
    """
    if not isinstance(Y_test, type(np.array([]))):  # Check data format
...
class VariableImportance:
    ...
    def one_variable_out(self):
        ...
        p = multiprocessing.Pool(
            None, maxtasksperchild=1)  # Multi-thread processing
        variables = list(self.m_x_train.columns)
        # [[v for v in method.variables if v != variable] for variable in method.variables])
        results = p.map(self.roc_for_variable_set, variables)
        sorted_variables_with_results = list(
            sorted(zip(variables, results), key=lambda x: x[1]))
        p.close()
    ...
    def recursive_one_variable_out(self):
        ...
        while len(remaining_variables) > 1:
            ...
            sorted_variables_with_results = list(
                sorted(zip(remaining_variables, results), key=lambda x: x[1]))
            removed_variables_with_results += sorted_variables_with_results[:1]
            remaining_variables = [v for v,
                                   r in sorted_variables_with_results[1:]]
        removed_variables_with_results += sorted_variables_with_results[1:]
        p.close()
    ...
    def permutation_feature(self):
        ...
        results = permutation_importance(
            ...
            random_state=None)
        variables = list(self.m_x_train.columns)
        sorted_variables_with_results = list(sorted(
            zip(variables, results.importances_mean), key=lambda x: x[1], reverse=True))
        return sorted_variables_with_results
    ...
    def shapley_values(self):
        ...
        method.fit(X_train, Y_train)
        if self.m_roc_area == "absv":
            def f(x): return method.decision_thresholds(x, glob_dec=True)
            method.clean()
        elif self.m_roc_area == "prob":
            def f(x): return method.predict_proba(x)[:, 1]
        elif self.m_roc_area == "deci":
            def f(x): return method.decision_function(x)
...
def plot_roc_curve(
    ...
    plt.plot(
        ...
        color='darkorange',
        lw=lw,
        label='ROC curve (area = %0.2f, N = %0.0f)' %
        (area,
         nClass),
        linestyle="-",
        marker="o")
    ...
    plt.savefig(
        ...
        '.png')
    output = pd.DataFrame(
        {'False positive rate': FPR, 'True positive rate': TPR, 'Area': area})
